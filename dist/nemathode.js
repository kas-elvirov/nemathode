...
(global = typeof globalThis !== 'undefined' ? globalThis : global || self, global.Nemathode = factory());
...
function isEvaluationFinished(ast, state) {
  return state.operandsProcessedAlready >= ast.length;
}
...
var evaluateAstWithInitialState = function evaluateAstWithInitialState(ast, config) {
  return evaluateAst(_objectSpread2({}, INITIAL_STATE), ast, config);
};
var evaluateAst = function evaluateAst(state, ast, config) {
  while (false === isEvaluationFinished(ast, state)) {
...
var _Nemathode = function _Nemathode(config) {
  ...
  this.evaluate = function (expression) {
    var ast = getAst(expression, _objectSpread2({}, _this.config));
    var result = evaluateAstWithInitialState([].concat(ast), _objectSpread2({}, _this.config));
    return result;
  };
